...
# It applies all migrations in order with proper tracking

echo "Running database initialization and migrations with tracking..."

# First, create migration tracking table
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    -- Check if this is an upgrade from Alembic
    DO \$\$
    BEGIN
        IF EXISTS (SELECT 1 FROM information_schema.tables 
            ...
            -- Mark all current tables as migrated
            INSERT INTO schema_migrations (version, description) 
            VALUES 
                ('base-schema', 'Initial database schema'),
                ('add-media-library', 'Media library tables'),
                ('add-pgvector-knowledge-base', 'Knowledge base with pgvector'),
                ('add-env-vars-table', 'Environment variables table'),
                ('add-call-summaries', 'Call summaries table')
            ON CONFLICT (version) DO NOTHING;
            
            RAISE NOTICE 'Imported existing schema from Alembic';
        END IF;
    END \$\$;
EOSQL
...
migration_applied() {
    local version=$1
    local count=$(psql -t -A --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -c \
...
# Base schema
apply_migration "base-schema" "Initial database schema" "
...
# Media library
apply_migration "add-media-library" "Media library tables" "
...
# Knowledge base
apply_migration "add-pgvector-knowledge-base" "Knowledge base with pgvector" "
...
apply_migration "add-env-vars-table" "Environment variables table" "
...
# Call summaries
apply_migration "add-call-summaries" "Call summaries table" "
...
# Agent type (Bedrock support)
apply_migration "add-agent-type" "Add Bedrock agent type support" "
    ALTER TABLE agents 
