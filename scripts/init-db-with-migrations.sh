...
echo "Base schema created. Running migrations..."

# Run migrations in order
# Each migration checks if it needs to run to be idempotent

# Migration: add-media-library.sql
if ! psql -t -A --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -c "SELECT 1 FROM schema_migrations WHERE version = 'add-media-library';" | grep -q 1; then
echo "Applying migration: add-media-library"
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    -- Record migration
    INSERT INTO schema_migrations (version) VALUES ('add-media-library') ON CONFLICT (version) DO NOTHING;
EOSQL
fi

# Migration: add-pgvector-knowledge-base.sql
if ! psql -t -A --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -c "SELECT 1 FROM schema_migrations WHERE version = 'add-pgvector-knowledge-base';" | grep -q 1; then
echo "Applying migration: add-pgvector-knowledge-base"
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    -- Record migration
    INSERT INTO schema_migrations (version) VALUES ('add-pgvector-knowledge-base') ON CONFLICT (version) DO NOTHING;
EOSQL
fi

# Migration: add-env-vars-table.sql
if ! psql -t -A --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -c "SELECT 1 FROM schema_migrations WHERE version = 'add-env-vars-table';" | grep -q 1; then
echo "Applying migration: add-env-vars-table"
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    -- Record migration
    INSERT INTO schema_migrations (version) VALUES ('add-env-vars-table') ON CONFLICT (version) DO NOTHING;
EOSQL
fi

# Migration: add call summaries tables
if ! psql -t -A --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -c "SELECT 1 FROM schema_migrations WHERE version = 'add-call-summaries';" | grep -q 1; then
echo "Applying migration: add-call-summaries"
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    -- Record migration
    INSERT INTO schema_migrations (version) VALUES ('add-call-summaries') ON CONFLICT (version) DO NOTHING;
EOSQL
fi

# Migration: add-agent-type.sql (NEW)
if ! psql -t -A --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -c "SELECT 1 FROM schema_migrations WHERE version = 'add-agent-type';" | grep -q 1; then
echo "Applying migration: add-agent-type"
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    -- Record migration
    INSERT INTO schema_migrations (version) VALUES ('add-agent-type') ON CONFLICT (version) DO NOTHING;
EOSQL
fi

# Migration: add-kb-settings-stats.sql
if ! psql -t -A --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -c "SELECT 1 FROM schema_migrations WHERE version = 'add-kb-settings-stats';" | grep -q 1; then
echo "Applying migration: add-kb-settings-stats"
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    -- Add settings and stats columns if they don't exist
    ...
    -- Record migration
    INSERT INTO schema_migrations (version) VALUES ('add-kb-settings-stats') ON CONFLICT (version) DO NOTHING;
EOSQL
fi
...
psql --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" -c "SELECT version, applied_at FROM schema_migrations ORDER BY applied_at;"